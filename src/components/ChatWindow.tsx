...
const AudioAttachmentPlayer = ({ url, name }: { url: string; name: string }) => {
  ...
  return (
    <div className="flex items-center gap-3 bg-white/70 border-2 border-black rounded-full px-4 py-2">
      <button
        onClick={togglePlayback}
        className="h-10 w-10 flex items-center justify-center rounded-full bg-vibrant-purple hover:bg-vibrant-pink transition-colors border-2 border-black"
        aria-label={isPlaying ? 'Pause audio message' : 'Play audio message'}
      >
        {isPlaying ? <FaPause className="w-4 h-4 text-black" /> : <FaPlay className="w-4 h-4 text-black" />}
      </button>
      <div className="flex-1 min-w-0">
        <span className="text-xs font-bold text-black truncate">{name}</span>
      </div>
...
const extractSymbolFromCoinType = (coinType: string) => {
  ...
  const handleGiftSend = async () => {
    ...
    try {
      ...
      await refreshTokens();
      setToastMessage(
        `Gifted ${formatBalance(amountAtomic, token.decimals)} ${token.symbol} to ${shortAddress(
          effectiveRecipient,
        )}.`,
      );
    ...
    <div className="w-full h-full text-black bg-white flex flex-col">
      <div className="px-6 py-4 bg-vibrant-blue flex items-center gap-4 shrink-0 border-b-4 border-black">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-2 w-40 rounded-[1.25rem] border-4 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,0.2)] z-30 overflow-hidden">
              <button
                onClick={handleGiftOption}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm font-black text-black hover:bg-vibrant-yellow transition-colors"
              >
                <FaGift className="w-4 h-4 text-black" />
                <span>Send Gift</span>
              </button>
            </div>
          )}
      ...
      <div className="px-4 py-4 bg-vibrant-blue flex items-center gap-3 shrink-0 border-t-4 border-black">
        ...
        <button 
          className="p-3 rounded-full cursor-pointer bg-white hover:bg-vibrant-yellow transition-colors border-4 border-black disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <form onSubmit={handleSendMessage} className="flex-1 flex items-center gap-3 relative">
          <div className="relative flex-1">
            <input
              ...
              className="w-full rounded-full titillium-web-bold bg-white text-black placeholder-black/60 pr-14 pl-5 py-3 outline-none border-4 border-black focus:bg-vibrant-mint transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            />
            <button
              type="button"
              onClick={() => setShowEmojiPicker(!showEmojiPicker)}
              className="absolute inset-y-0 right-3 my-auto h-10 w-10 rounded-full bg-white hover:bg-vibrant-yellow transition-colors border-4 border-black flex items-center justify-center"
            >
          ...
          {hasSendableContent ? (
            ...
            <button
              ...
              className={`p-4 cursor-pointer rounded-full border-4 border-black transition-colors ${
                isRecording
                  ? 'bg-vibrant-orange hover:bg-vibrant-pink animate-pulse'
                  : 'bg-white hover:bg-vibrant-yellow'
              }`}
              disabled={!isReady || isSendingMessage}
            >
              {isRecording ? (
                <FaStop className="w-5 h-5 text-black" />
              ) : (
      ...
      {isGiftModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-[2rem] border-4 border-black bg-white p-6 shadow-[12px_12px_0px_0px_rgba(0,0,0,0.2)]">
            <h2 className="text-xl font-black text-black mb-4 text-center">Send a Gift</h2>
            {otherParticipants.length > 1 ? (
              <div className="space-y-3">
                <label className="block text-sm font-bold text-black">Select recipient</label>
                <select
                  ...
                  className="w-full rounded-full border-4 border-black bg-white px-4 py-3 text-sm font-bold text-black focus:outline-none focus:bg-vibrant-mint"
                >
            ...
            {isLoadingTokens && (
              <p className="mt-3 text-xs font-bold text-black/60 text-center">Loading wallet tokens...</p>
            )}

            <div className="mt-4 space-y-3">
              <label className="block text-sm font-bold text-black">Token</label>
              <select
                ...
                className="w-full rounded-full border-4 border-black bg-white px-4 py-3 text-sm font-bold text-black focus:outline-none focus:bg-vibrant-mint disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            {giftError && (
              <div className="mt-3 rounded-full border-4 border-black bg-vibrant-yellow px-4 py-2 text-sm font-bold text-black text-center">
                {giftError}
              </div>
            )}
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button
                ...
                className="rounded-full border-4 border-black bg-white px-5 py-2 text-sm font-bold text-black hover:bg-vibrant-yellow transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-full border-4 border-black bg-vibrant-purple px-5 py-2 text-sm font-bold text-black hover:bg-vibrant-pink transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
